...
diffc=`diff --ignore-all-space tmp/example_data.out exp_long_xlen/example_data.out_EXP | wc -l`
if test ! $diffc = 0
then
	echoerr "error: (1), diff tmp/example_data.out exp_long_xlen/example_data.out_EXP"
        popd > /dev/null
	exit 1
fi
...
diffc=`diff --ignore-all-space tmp/example_data.scores exp_long_xlen/example_data.scores_EXP | wc -l`
if test ! $diffc = 0
then
	echoerr "error: (2), diff tmp/example_data.scores exp_long_xlen/example_data.scores_EXP"
        popd > /dev/null
	exit 2
fi
...
echo "../bin/tlsh_unittest -xlen -l tmp/example_data.tlsh -c ../Testing/example_data/website_course_descriptors06-07.txt > tmp/example_data.scores.2"
      ../bin/tlsh_unittest -xlen -l tmp/example_data.tlsh -c ../Testing/example_data/website_course_descriptors06-07.txt > tmp/example_data.scores.2

diffc=`diff --ignore-all-space tmp/example_data.scores.2 exp_long_xlen/example_data.scores.2_EXP | wc -l`
if test ! $diffc = 0
then
	echoerr "error: (3) diff tmp/example_data.scores.2 exp_long_xlen/example_data.scores.2_EXP"
        popd > /dev/null
	exit 3
fi
