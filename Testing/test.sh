...
diffc=`diff --ignore-all-space tmp/example_data.out exp/example_data.out_EXP | wc -l`
if test ! $diffc = 0
then
	echoerr "error: (1), diff tmp/example_data.out exp/example_data.out_EXP"
        popd > /dev/null
	exit 1
fi
...
diffc=`diff --ignore-all-space tmp/example_data.scores exp/example_data.scores_EXP | wc -l`
if test ! $diffc = 0
then
	echoerr "error: (2), diff tmp/example_data.scores exp/example_data.scores_EXP"
        popd > /dev/null
	exit 2
fi
...
diffc=`diff --ignore-all-space tmp/example_data.scores.2 exp/example_data.scores.2_EXP | wc -l`
if test ! $diffc = 0
then
	echoerr "error: (3) diff tmp/example_data.scores.2 exp/example_data.scores.2_EXP"
        popd > /dev/null
	exit 3
fi
