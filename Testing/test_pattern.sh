...
for fieldSep in TAB COMMA ; do
	...
	testnum=1
	echo
	echo "test_pattern $testnum using seperator $fieldSep"
	echo
	...
	#	col 5: pattern label
	############
	if test $fieldSep = "TAB"
	then
		PATTERN_FILE=tmp/tenfile.pat
		EXPECTED_PATFILE=exp/tenfile.pat_EXP
		SEP="	"
	else
		PATTERN_FILE=tmp/tencomma.pat
		EXPECTED_PATFILE=exp/tencomma.pat_EXP
		SEP=","
	fi
	rm -f $PATTERN_FILE
	echo "pattern_number${SEP}nitems_in_group${SEP}TLSH${SEP}radius${SEP}pattern_label" > $PATTERN_FILE
	patn=0
	for f in	021106_yossivassa.txt 0Alice.txt 11-17-06_Academy.txt 1english-only.txt 2005NISSE.txt			\
		...
		echo "pat_$patn${SEP}1${SEP}$tlsh${SEP}30${SEP}$f"	>> $PATTERN_FILE
		patn=`expr $patn + 1`
	done
